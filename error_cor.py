...
def get_arguments():
    ...
    parser.add_argument("--start_poi", type=int, default=55019017)
    parser.add_argument("--end_poi", type=int, default=55211628)
    parser.add_argument("--read_L", type=int, default=101)
    #gene_matched_20.sam
    parser.add_argument("--sam_file", type=str, default='g_matched_1.sam')
    parser.add_argument("--write_file", type=str, default='corrected_1.fa')
    parser.add_argument("--write_file_2in", type=str, default='corrected_1_2in.txt')
    parser.add_argument("--write_file_seq", type=str, default='updateseq_1.fa')
...
#setting
gene_length = args.end_poi - args.start_poi + 1
...
#for i in range(619:622):
    ...
    for j in range(len(sam_q)):
        try:
            ...
            pdb.set_trace()
    #store info into list for input of sparse matrix
    val_l.extend(sam_q_num)
    #t = np.array(sam_q_num)
    #A[i,index:index+100] = t.T
...
for i in range(51,gene_length+51):
    ...
    #Num_col = Num_col + col_num   
    #num_col_list.append(col_num)
    
    #maximum letter's number is max_l + 1
    if(tmp_count.shape[0] == 0):
        #0 means this base is unknown
        print(" %d there is some base cannot be update" % i)
        Seq_tru.append(5)
        continue
    else:      
...
print("error correction finished",Num_errors)
...
#store _2in.txt for evaluation
W_2in_file = args.write_file_2in
in2_file = open(W_2in_file,'w') 
...
print("%d reads are stored in the file" % read_sto)
print("%d reads are labeled from other genes" % int(i-read_sto))
fa_file.close()
in2_file.close()
